...
function radarSetSpeed(defaultText)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString("~r~Bitte gib die korrekte Nummer ein !")
...
function POLICE_radar()
    ...
    if isRadarPlaced then 
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString("~r~Du befindest dich nicht neben dem Radar !")
            DrawSubtitleTimed(3000, 1)
    ...
    while true do
        ...
        if isRadarPlaced then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) > 300 then 
                ...
                ClearPrints()
                SetTextEntry_2("STRING")
                AddTextComponentString("~r~Du bist zu weit von deinem Radar entfernt !")
                DrawSubtitleTimed(3000, 1)
        ...
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format("   Fahrzeug ~r~%s ~w~Kennzeichen ~r~%s ~w~Km/h ~g~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format("   Fahrzeug ~r~%s ~w~Kennzeichen ~r~%s ~w~Km/h ~r~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format("   Fahrzeug ~r~%s ~w~Kennzeichen ~r~%s ~w~Km/h ~r~%s", LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end
